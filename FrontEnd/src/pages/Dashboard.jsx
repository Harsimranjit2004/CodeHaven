...
const Dashboard = () => {
      ...
      try {
        ...
        const fetchedProjects = response.data.map(project => {
          ...
          return {
            ...
            latest: project.latest || `Last updated - ${new Date(project.lastDeployed).toLocaleDateString()}`,
            lastDeployed: new Date(project.lastDeployed).toLocaleDateString(),
        ...
        setToasts(prev => [...prev, { id: Date.now(), message: "Error fetching projects" }]);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    socket.current.on("connect", () => {
      ...
      projects.forEach(project => socket.current.emit("subscribe", project.id));
    });

    socket.current.on("status-update", ({ projectId, status, message }) => {
      setProjects(prev =>
        prev.map(project =>
          project.id === projectId
            ? { ...project, status, latest: message || project.latest }
            : project
        )
      );
      setToasts(prev => [...prev, { id: Date.now(), message: `${projectId}: ${status}` }]);
    });
  ...
  const sortedProjects = [...filteredProjects].sort((a, b) => {
    if (sortOption === "activity") {
      return new Date(b.latest.split(" - ")[1] || b.lastDeployed) - new Date(a.latest.split(" - ")[1] || a.lastDeployed);
    } else if (sortOption === "name") {
      return a.name.localeCompare(b.name);
    } else if (sortOption === "status") {
      return a.status.localeCompare(b.status);
  ...
  // Status styles with lowercase keys to match backend
  const statusStyles = {
    "dev contact": "bg-yellow-600 text-white", // Yellow background (#D97706)
    deployed: "bg-green-600 text-white", // Green background (#10B981)
    building: "bg-purple-600 text-white", // Purple background
    failed: "bg-red-600 text-white",
    uploading: "bg-blue-600 text-white", // Blue background for "uploading"
    unknown: "bg-gray-500/20 text-gray-400", // Default for unknown status
  };

  // Framework icons with green color (#10B981)
  const frameworkIcons = {
    "Next.js": <Zap className="h-5 w-5 text-green-500" />,
    React: <Code className="h-5 w-5 text-green-500" />,
    "Node.js": <Activity className="h-5 w-5 text-green-500" />,
    "Vanilla JS": <Globe className="h-5 w-5 text-green-500" />,
    Unknown: <Code className="h-5 w-5 text-green-500" />,
  };
  ...
  const handleRedeploy = async (projectId) => {
    try {
      await axios.post(`${BACKEND_URL}/deploy`, {
        ...
        repoUrl: projects.find(p => p.id === projectId)?.repoUrl,
        projectName: projects.find(p => p.id === projectId)?.name,
      });
      addToast(`Redeploying ${projectId}`);
    } catch (error) {
      addToast(`Failed to redeploy ${projectId}`);
    }
  };
    ...
    <div className={`h-screen w-screen flex font-sans overflow-hidden ${theme === "dark" ? "bg-gradient-to-br from-gray-900 to-black text-white" : "bg-gray-100 text-black"}`}>
      ...
      <main className="flex-1 h-full overflow-y-auto lg:pt-0 pt-16">
        <div className="sticky top-0 z-10 bg-gray-900/95 backdrop-blur-md p-6 border-b border-gray-700 lg:pt-6 dark:bg-gray-900/95 dark:border-gray-700">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <select
                  ...
                  className="bg-gray-800/50 border border-gray-700 text-white appearance-none rounded-lg px-4 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm dark:bg-gray-800/50 dark:border-gray-700 dark:text-white"
                >
                  <option value="activity" className="bg-gray-900 text-white">Sort by Activity</option>
                  <option value="name" className="bg-gray-900 text-white">Sort by Name</option>
                  <option value="status" className="bg-gray-900 text-white">Sort by Status</option>
                </select>
                <svg
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-500 pointer-events-none"
                  fill="none"
          ...
          <div className="flex flex-wrap items-center space-x-2 mt-4 overflow-x-auto">
            {["all", "deployed", "building", "failed", "uploading", "dev contact"].map((status) => (
              <motion.button
                key={status}
                whileHover={{ scale: 1.05, y: -2 }}
                whileTap={{ scale: 0.95 }}
            ...
            {loading ? (
              Array.from({ length: 3 }).map((_, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 0.3 }}
                  transition={{ duration: 0.5, repeat: Infinity, repeatType: "reverse" }}
                  className="h-36 bg-[#1A2526] rounded-xl border border-[#2A3536]"
                />
              ))
              ...
              sortedProjects.map((project, index) => (
                <motion.div
                  ...
                  whileHover={{
                    scale: 1.02,
                    boxShadow: "0 10px 20px rgba(0, 0, 0, 0.3), 0 0 20px rgba(59, 130, 246, 0.2)",
                  }}
                  className="bg-[#1A2526] border border-[#2A3536] rounded-xl p-4 flex flex-col justify-between hover:bg-[#202B2C] transition-all cursor-pointer group"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      {/* Left-side framework icon */}
                      {frameworkIcons[project.framework] || frameworkIcons["Unknown"]}
                      <div>
                        <div className="flex items-center space-x-2">
                          <a
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[#00BFFF] text-sm underline"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {project.domain}
                        </a>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <motion.span
                        ...
                        animate={{ scale: 1, opacity: 1 }}
                        transition={{ duration: 0.3 }}
                        className={`px-3 py-1 rounded-full text-xs font-medium ${statusStyles[project.status] || "bg-gray-500/20 text-gray-400"}`}
                      >
                        {project.status.charAt(0).toUpperCase() + project.status.slice(1)} {/* Capitalize for display */}
                      </motion.span>
                      <div className="relative">
                        <motion.button
                          whileHover={{ scale: 1.1, rotate: 90 }}
                        ...
                        <motion.div
                          ...
                          whileHover={{ opacity: 1, y: 0 }}
                          transition={{ duration: 0.2 }}
                          className="absolute right-0 top-8 bg-[#1A2526] border border-[#2A3536] rounded-xl shadow-lg p-2 space-y-2 hidden group-hover:block z-40"
                        >
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="flex items-center space-x-2 px-4 py-2 text-sm text-gray-300 hover:bg-[#202B2C] hover:text-white rounded-lg w-full text-left transition-all z-30"
                          >
                            <RefreshCw className="h-4 w-4" />
                            <span>Redeploy</span>
                          </button>
                          ...
                          <button
                            ...
                            className="flex items-center space-x-2 px-4 py-2 text-sm text-red-400 hover:bg-[#202B2C] hover:text-red-300 rounded-lg w-full text-left transition-all z-30"
                          >
                            <Trash2 className="h-4 w-4" />
                            <span>Delete</span>
                          </button>
                  ...
                  <div>
                    <p className="text-gray-400 text-sm mt-1">{project.latest}</p>
                  </div>
                </motion.div>
              ))
            ) : (
